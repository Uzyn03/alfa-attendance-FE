  ...
  <div class="min-h-screen bg-background">
      ...
      <div class="container mx-auto px-4 py-6">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-3">
            ...
            <div>
              <h1 class="text-2xl font-bold text-foreground">Employee Analytics</h1>
              <p class="text-sm text-muted-foreground">Performance Prediction System</p>
            </div>
          </div>
          <div class="flex items-center space-x-4">
            <div class="flex items-center space-x-2 text-sm text-muted-foreground">
              <div class="h-2 w-2 animate-pulse rounded-full" :class="firebaseStatus.connected ? 'bg-green-500' : 'bg-red-500'"></div>
              <span>{{ firebaseStatus.connected ? 'Firebase Connected' : 'Firebase Disconnected' }}</span>
            </div>
            <Button @click="refreshData" :disabled="isRefreshing" variant="outline" size="sm">
              <RefreshCw :class="{ 'animate-spin': isRefreshing }" class="mr-2 h-4 w-4" />
              Refresh
            </Button>
          </div>
        </div>
    ...
    <div class="container mx-auto space-y-8 py-8 px-4 md:px-6">
      <div class="grid grid-cols-1 gap-6 md:grid-cols-4">
        <Card class="border-0 bg-gradient-to-r from-blue-500 to-blue-600 text-white">
          <CardContent class="p-6">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm font-medium text-blue-100">Total Predictions</p>
        ...
        <Card class="border-0 bg-gradient-to-r from-green-500 to-green-600 text-white">
          <CardContent class="p-6">
            <div class="flex items-center justify-between">
              <div>
        ...
        <Card class="border-0 bg-gradient-to-r from-purple-500 to-purple-600 text-white">
          <CardContent class="p-6">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm font-medium text-purple-100">Avg Performance</p>
        ...
        <Card class="border-0 bg-gradient-to-r from-orange-500 to-orange-600 text-white">
          <CardContent class="p-6">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-sm font-medium text-orange-100">Last Updated</p>
                <ClientOnly>
                  <p class="text-lg font-bold">{{ formatTime(stats.lastUpdated) }}</p>
                  <template #fallback>
      ...
      <Card class="border shadow-lg">
        ...
        <CardContent class="space-y-6 p-8">
          <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            <div v-for="(config, key) in inputConfigs" :key="key" class="space-y-3">
              <Label :for="key" class="flex items-center space-x-2 text-sm font-semibold">
              ...
              <div class="h-2 w-full rounded-full bg-secondary">
                <div 
                  ...
                  :style="{ width: `${Math.min((regressionFeatures[key] / config.max) * 100, 100)}%` }"
                ></div>
              </div>
            </div>
          </div>
          ...
          <div class="flex flex-col gap-4 sm:flex-row">
            ...
            <Button
              ...
              :disabled="!regressionPrediction || savingToFirestore || !firebaseStatus.connected"
              variant="outline"
              size="lg"
          ...
          <div v-if="errorRegression" class="rounded-lg border border-destructive/50 bg-destructive/10 p-4">
          ...
          <div v-if="firebaseError" class="rounded-lg border border-destructive/50 bg-destructive/10 p-4">
            <div class="flex items-center">
              <AlertCircle class="mr-2 h-5 w-5 text-destructive" />
              <p class="font-medium text-destructive">Firebase Error: {{ firebaseError }}</p>
            </div>
          </div>

          <div v-if="successMessage" class="rounded-lg border border-green-500/50 bg-green-50 p-4 dark:bg-green-950">
            <div class="flex items-center">
              <CheckCircle class="mr-2 h-5 w-5 text-green-600" />
              <p class="font-medium text-green-700 dark:text-green-300">{{ successMessage }}</p>
            </div>
          </div>

          <div v-if="regressionPrediction" class="rounded-xl border-2 border-green-200 bg-green-50 p-6 transition-all duration-500 hover:shadow-lg dark:border-green-800 dark:bg-green-950">
            <div class="space-y-4 text-center">
              <div class="flex items-center justify-center space-x-2">
                <CheckCircle class="h-8 w-8 text-green-600" />
                <h3 class="text-xl font-bold text-green-800 dark:text-green-200">Prediction Result</h3>
              </div>
                ...
                <CardContent class="p-6">
                  <div class="mb-4 flex items-center justify-center space-x-4">
                    <div class="text-center">
                      <p class="text-4xl font-bold">{{ regressionPrediction.predicted_score.toFixed(1) }}</p>
                      <p class="text-sm text-muted-foreground">Performance Score</p>
                    </div>
                    <Separator orientation="vertical" class="h-16" />
                    <div class="text-center">
                      <p class="text-2xl font-bold" :class="getCategoryColor(regressionPrediction.category)">
                  ...
                  <div class="mb-4 h-4 w-full rounded-full bg-secondary">
                    <div 
                      ...
                      :class="getScoreBarColor(regressionPrediction.predicted_score)"
                      :style="{ width: `${Math.min(regressionPrediction.predicted_score, 100)}%` }"
                    ></div>
                  </div>
                  
                  <p class="text-center font-medium text-green-700 dark:text-green-300">{{ regressionPrediction.message }}</p>
                </CardContent>
              </Card>
            </div>
          </div>
      ...
      <Card class="border shadow-lg">
        <CardHeader class="bg-gradient-to-r from-purple-600 to-pink-600 text-white">
          <CardTitle class="flex items-center space-x-2">
        ...
        <CardContent class="space-y-6 p-8">
          ...
          <div v-if="errorModelStatus" class="rounded-lg border border-destructive/50 bg-destructive/10 p-4">
            <div class="flex items-center">
              <AlertCircle class="mr-2 h-5 w-5 text-destructive" />
          ...
          <div v-if="modelStatus && modelStatus.model_trained" class="space-y-8">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
              <Card class="border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-950">
                <CardContent class="p-4">
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="text-sm font-medium text-blue-600 dark:text-blue-400">Training R²</p>
                      <p class="text-2xl font-bold text-blue-800 dark:text-blue-200">
                        {{ modelStatus.model_metadata?.rf_train_r2?.toFixed(3) || "N/A" }}
                      </p>
                    </div>
              ...
              <Card class="border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-950">
                <CardContent class="p-4">
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="text-sm font-medium text-green-600 dark:text-green-400">Test R²</p>
                      <p class="text-2xl font-bold text-green-800 dark:text-green-200">
                        {{ modelStatus.model_metadata?.rf_test_r2?.toFixed(3) || "N/A" }}
                      </p>
                    </div>
              ...
              <Card class="border-purple-200 bg-purple-50 dark:border-purple-800 dark:bg-purple-950">
                <CardContent class="p-4">
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="text-sm font-medium text-purple-600 dark:text-purple-400">Data Size</p>
              ...
              <Card class="border-orange-200 bg-orange-50 dark:border-orange-800 dark:bg-orange-950">
                <CardContent class="p-4">
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="text-sm font-medium text-orange-600 dark:text-orange-400">Status</p>
                      <p class="text-lg font-bold text-orange-800 dark:text-orange-200">
                        {{ getOverfittingStatus(modelStatus.model_metadata?.rf_overfitting_ratio) }}
                      </p>
                    </div>
                    <CheckCircle class="h-8 w-8 text-orange-500" />
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            <div class="space-y-6">
              ...
              <div class="grid grid-cols-1 gap-8 lg:grid-cols-2">
                <Card v-if="modelStatus.feature_importance_plot" class="overflow-hidden">
                  <CardHeader class="bg-gradient-to-r from-blue-500 to-indigo-500 text-white">
                  ...
                  <CardContent class="p-4">
                    <img
                      ...
                      class="h-auto w-full rounded-lg transition-transform duration-300 hover:scale-105"
                      :key="visualizationKey"
                    />
                  </CardContent>
                </Card>
                
                <Card v-if="modelStatus.actual_vs_predicted_plot" class="overflow-hidden">
                  <CardHeader class="bg-gradient-to-r from-green-500 to-emerald-500 text-white">
                    <CardTitle class="flex items-center space-x-2 text-base">
              ...
              <Card v-if="modelStatus.predicted_distribution_plot" class="overflow-hidden">
                <CardHeader class="bg-gradient-to-r from-purple-500 to-pink-500 text-white">
                  <CardTitle class="flex items-center space-x-2 text-base">
                    <Activity class="h-5 w-5" />
                    <span>Score Distribution Analysis</span>
                  </CardTitle>
                </CardHeader>
                <CardContent class="p-4">
                  <img
          ...
          <div v-else-if="!loadingModelStatus && !errorModelStatus" class="py-12 text-center">
            <div class="space-y-4">
              <AlertCircle class="mx-auto h-16 w-16 text-muted-foreground" />
              <h3 class="text-xl font-semibold text-muted-foreground">No Model Data Available</h3>
              <p class="text-muted-foreground">Train the model first to see analytics and visualizations</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card class="border shadow-lg">
        <CardHeader class="bg-gradient-to-r from-indigo-600 to-purple-600 text-white">
          <CardTitle class="flex items-center space-x-2">
        ...
        <CardContent class="p-6">
          <div v-if="predictionHistory.length === 0" class="py-8 text-center">
            ...
            <p class="text-muted-foreground">No predictions yet. Make your first prediction above!</p>
          </div>
          <div v-else class="space-y-4">
            ...
            >
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                  <div class="flex h-12 w-12 items-center justify-center rounded-full font-bold text-white"
                       :class="getScoreBackgroundColor(prediction.score)">
                    {{ prediction.score.toFixed(0) }}
                  </div>
                  <div>
                    <p class="font-semibold">Score: {{ prediction.score.toFixed(1) }}</p>
                    <p class="text-sm text-muted-foreground">Category: {{ prediction.category }}</p>
                  </div>
                </div>
                <div class="text-right">
                  <ClientOnly>
                    <p class="text-sm text-muted-foreground">{{ formatTime(prediction.timestamp) }}</p>
                    <template #fallback>
...
const handlePredictRegression = async () => {
  ...
  try {
    ...
    stats.value.avgPerformance = predictionHistory.value.reduce((sum, p) => sum + p.score, 0) / predictionHistory.value.length;
    stats.value.lastUpdated = new Date();
    
  } catch (err) {
    errorRegression.value = err.message || "An unexpected error occurred during prediction.";
...
const fetchModelStatus = async () => {
  ...
  try {
    ...
    if (data.model_metadata?.rf_test_r2) {
      stats.value.modelAccuracy = Math.round(data.model_metadata.rf_test_r2 * 100);
    }
    
  } catch (err) {
    errorModelStatus.value = err.message || "An error occurred while fetching model status.";
...
const handleRetrainModel = async () => {
  ...
  try {
    ...
    successMessage.value = "Model successfully retrained! Loading latest model status...";
    ...
    errorModelStatus.value = err.message || "An error occurred while retraining the model.";
...
const saveToFirestore = async () => {
  if (!regressionPrediction.value) {
    firebaseError.value = 'Tidak ada prediksi untuk disimpan. Buat prediksi terlebih dahulu.';
    setTimeout(() => {
      firebaseError.value = null;
    }, 3000);
    return;
  ...
  try {
    ...
    successMessage.value = `Prediksi berhasil disimpan ke database! (ID: ${docId.substring(0, 8)}...)`;
...
const refreshData = async () => {
  ...
  try {
    ...
    await Promise.all([
      fetchModelStatus().catch(err => console.warn('Warning fetch model status:', err)),
      checkFirebaseStatus().catch(err => console.warn('Warning check Firebase:', err)),
      loadPredictionsFromFirestore().catch(err => console.warn('Warning load predictions:', err))
    ]);
...
const loadPredictionsFromFirestore = async () => {
  try {
    ...
    console.log(`✅ Berhasil memuat ${predictions.length} prediksi dari Firestore`);
    
    // Update stats
    if (formattedPredictions.length > 0) {
      stats.value.totalPredictions = formattedPredictions.length;
      stats.value.avgPerformance = formattedPredictions.reduce((sum, p) => sum + p.score, 0) / formattedPredictions.length;
    }
...
const getCategoryColor = (category) => {
  switch (category.toLowerCase()) {
    case 'baik': return 'text-green-600 dark:text-green-400';
    case 'cukup': return 'text-yellow-600 dark:text-yellow-400';
    case 'kurang': return 'text-red-600 dark:text-red-400';
